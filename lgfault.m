% O programa lgfault é projetado para a análise de faltas monofásicas
% para terra em uma rede de sistema elétrico de potência. O programa
% requer as matrizes de impedância de barras de sequência positiva,
% negativa e zero, Zbus1, Zbus2 e Zbus0. As matrizes de impedância de barras 
% podem ser definidas pelo usuário, obtidas pela inversão de Ybus ou determinadas 
% pelas funções Zbus = zbuild(zdata) ou Zbus = zbuildpi(linedata, gendata, yload).
...
fprintf('\nAnálise de falta monofásica para terra \n')
ff = 999;
while ff > 0
    ...
    fprintf('  \n')
    fprintf('Falta monofásica para terra na barra número %g\n', nf)

    a = cos(2*pi/3) + j*sin(2*pi/3);
    sctm = [1   1   1; 1 a^2  a; 1 a  a^2];
    Ia0 = V0(nf) / (Zbus1(nf,nf) + Zbus2(nf,nf) + Zbus0(nf,nf) + 3*Zf); 
    Ia1 = Ia0; 
    Ia2 = Ia0;
    I012 = [Ia0; Ia1; Ia2];
    Ifabc = sctm * I012;
